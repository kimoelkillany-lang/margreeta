...
function GenZStopCard({ s, onNav }) {
  ...
  return (
    <div onClick={handleClick} onMouseEnter={() => canHover && setFlipped(true)} onMouseLeave={() => canHover && setFlipped(false)}
    ...
    >
      <div style={{ position: 'relative', width: '100%', height: '100%', transformStyle: 'preserve-3d', transition: 'transform .6s var(--ease-bounce)', transform: flipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
        ...
        <div style={{ position: 'absolute', inset: 0, backfaceVisibility: 'hidden', transform: 'rotateY(180deg)', borderRadius: 'var(--radius-md)', overflow: 'hidden', boxShadow: 'var(--shadow-card)' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(0,0,0,.05) 35%, rgba(0,0,0,.88) 100%)', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', gap: 4, padding: 20 }}>
            ...
            {!canHover && <div style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'rgba(255,255,255,.85)', letterSpacing: '0.02em' }}>Tap again to explore →</div>}
          </div>
        </div>
      </div>
    </div>
