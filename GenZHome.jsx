...
function GenZStopCard({ s, onNav }) {
  const { JourneyStamp, Postcard } = window.MargreetaDesignSystem_35c101;
  const [hover, setHover] = React.useState(false);
  return (
    <div onClick={() => onNav(s.key)} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{ cursor: 'pointer', height: 280, perspective: 1200 }}
    >
      <div style={{ position: 'relative', width: '100%', height: '100%', transformStyle: 'preserve-3d', transition: 'transform .6s var(--ease-bounce)', transform: hover ? 'rotateY(180deg)' : 'rotateY(0deg)' }}>
        <div style={{ position: 'absolute', inset: 0, backfaceVisibility: 'hidden' }}>
          <Postcard eyebrow={`Stop No. ${s.number} — ${s.key[0].toUpperCase() + s.key.slice(1)}`} tone="white" style={{ height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', boxShadow: hover ? 'var(--shadow-card)' : 'var(--shadow-rest)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 12 }}>
              <div>
                <div style={{ fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: 19, color: 'var(--ink-black)' }}>{s.title}</div>
                <div style={{ fontFamily: 'var(--font-script)', fontStyle: 'italic', fontSize: 15, color: 'var(--text-muted-on-light)', marginTop: 10 }}>{s.line}</div>
              </div>
              <div style={{ position: 'relative', width: 64, height: 64, flexShrink: 0 }}>
                <span style={{ position: 'absolute', inset: -16, borderRadius: '50%', background: 'var(--gold-highlight)', transform: hover ? 'scale(1)' : 'scale(0)', opacity: hover ? 0.4 : 0, transition: 'transform .5s var(--ease-bounce), opacity .4s var(--ease-smooth)' }}></span>
                <JourneyStamp country={s.key} number={s.number} size={64} />
              </div>
            </div>
          </Postcard>
        </div>
        <div style={{ position: 'absolute', inset: 0, backfaceVisibility: 'hidden', transform: 'rotateY(180deg)', borderRadius: 'var(--radius-md)', overflow: 'hidden', boxShadow: 'var(--shadow-card)' }}>
          ...
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(0,0,0,.05) 35%, rgba(0,0,0,.88) 100%)', display: 'flex', alignItems: 'flex-end', gap: 12, padding: 20 }}>
            <JourneyStamp country={s.key} number={s.number} size={48} />
            <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 20, color: '#fff', textTransform: 'capitalize' }}>{s.key}</div>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div>
      <section style={{ background: 'var(--surface-cream)', padding: '96px 48px 120px', textAlign: 'center' }}>
        ...
        <div style={{ maxWidth: 640, margin: '32px auto 0', textAlign: 'left', fontFamily: 'var(--font-body)', fontSize: 16, color: 'var(--text-on-red-muted)', lineHeight: 'var(--lh-body)' }}>
          ...
          <p style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 22, color: 'var(--text-on-red)', marginTop: 24 }}>The World Tour Concept</p>
          ...
          <p style={{ fontFamily: 'var(--font-script)', fontStyle: 'italic', fontSize: 19, color: 'var(--gold-highlight)', marginTop: 20 }}>One dough. Endless destinations.</p>
        </div>
      ...
      <section id="world-tour-section" style={{ background: 'var(--surface-cream)', padding: '80px 48px' }}>
        <div style={{ maxWidth: 720, margin: '0 auto 56px' }}>
          <SectionEyebrow accent="gold">The Margreeta World Tour</SectionEyebrow>
          <div style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 32, color: 'var(--text-on-red)' }}>Same dough. Every stamp a different country.</div>
        </div>
      ...
      <section id="concept-section" style={{ background: 'var(--surface-deep-red)', padding: '64px 48px', boxShadow: 'inset 0 12px 24px -12px rgba(0,0,0,.45), inset 0 -12px 24px -12px rgba(0,0,0,.45)', borderTop: '1px solid rgba(0,0,0,.35)', borderBottom: '1px solid rgba(0,0,0,.35)' }}>
        ...
        <div style={{ maxWidth: 1040, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(440px, 1fr))', gap: 24 }}>
          {[
            ...
            <GenZReveal key={item.key} delay={i * 0.06} style={{
            ...
            >
              <div style={{ position: 'relative', flexShrink: 0 }}>
                <image-slot id={`ingredient-${item.key}`} src={`assets/${item.image}`} placeholder={`Photo of ${item.title.toLowerCase()}`} shape="rounded" style={{ width: 108, height: 108, display: 'block' }}></image-slot>
                <span style={{ position: 'absolute', top: -8, left: -8, width: 28, height: 28, borderRadius: '50%', background: 'var(--gold-foil)', color: 'var(--ink-bordeaux-900)', fontFamily: 'var(--font-stamp)', fontSize: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 3px 8px rgba(0,0,0,.3)' }}>{item.num}</span>
              </div>
              <div>
                <div style={{ fontFamily: 'var(--font-body)', fontWeight: 700, fontSize: 18, color: 'var(--text-on-red)', marginBottom: 6 }}>{item.title}</div>
                <div style={{ fontFamily: 'var(--font-body)', fontSize: 14, color: 'var(--text-on-red-muted)', lineHeight: 1.45 }}>{item.copy}</div>
              </div>
            </GenZReveal>
          ))}
        </div>
